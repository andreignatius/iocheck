...
### 2026-08-01 19:50 — EXPERIMENT: latency sweep L=50ms (Andre; throwaway, auto-reverted)
Ran the latency-knob idea empirically on the live 0.1.7 cluster: patched ConfigMap `STORE_LOOKUP_LATENCY_MS` 700→50, restarted, ran the k6 storm while sampling replicas+CPU, captured k6 p99, then **auto-reverted to 700** (trap). Committed demo unchanged; cluster healthy at 700, 0 restarts. Evidence: [`logs/EXPERIMENT-latency-sweep.log`](../logs/EXPERIMENT-latency-sweep.log).
- **Validity:** inline env-check printed 700 (a `rollout restart`→`rollout status` race returning the old generation), but the storm ran at L=50 — median req **72ms** (impossible at 700ms) and CPU 4–8× the 700ms runs both confirm it.
